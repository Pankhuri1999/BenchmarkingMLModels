...
# Import general fairness improvement module
try:
    from general_fairness_improvement import (
        integrate_fairness_improvement_into_pipeline,
        improve_fairness_automatically
    )
    FAIRNESS_IMPROVEMENT_AVAILABLE = True
except ImportError:
    FAIRNESS_IMPROVEMENT_AVAILABLE = False
    print("⚠️ Warning: general_fairness_improvement module not available. Auto-fairness improvement disabled.")
...
def comprehensive_ml_pipeline(dataset_path, target_column, task_type='auto',
                             ...
                             max_tfidf_features=2000, demographic_column=None,
                             readiness_weights=None, readiness_sub_weights=None,
                             tfidf_min_df=5, tfidf_max_df=0.95,
                             n_estimators=100, max_depth=None, early_stopping=True,
                             n_jobs=-1, enable_checkpointing=False, checkpoint_dir='./checkpoints',
                             auto_improve_fairness=True, disparity_threshold=None,
                             **kwargs):
    """
    ...
    Args:
        ...
        max_tfidf_features: Maximum TF-IDF features (default: 2000 for speed)
        tfidf_min_df: Minimum document frequency for TF-IDF (filters rare terms, default: 5)
        tfidf_max_df: Maximum document frequency for TF-IDF (filters common terms, default: 0.95)
        n_estimators: Number of trees for tree-based models (default: 100 for speed)
        max_depth: Maximum depth for tree-based models (None = adaptive based on dataset size)
        early_stopping: Use early stopping for XGBoost and LightGBM (default: True)
        n_jobs: Number of parallel jobs (-1 = use all cores, default: -1)
        enable_checkpointing: Save intermediate results to prevent data loss (default: False)
        ...
        auto_improve_fairness: Automatically improve fairness if disparity is high (default: True)
                              Uses general techniques: post-processing calibration, model selection
        disparity_threshold: Threshold for high disparity (None = auto-detect based on task type)
                            For regression: 10% of target std
                            For classification: 5% accuracy difference
    ...
    if is_multi_target:
        ...
        for target_col in target_columns:
            ...
            if enable_checkpointing:
                import pickle
                checkpoint_file = os.path.join(checkpoint_dir, f'preprocessed_{target_col}.pkl')
                with open(checkpoint_file, 'wb') as f:
                    pickle.dump({
                        'X_train': X_train, 'X_test': X_test,
                        'y_train': y_train, 'y_test': y_test,
                        'feature_names': feature_names, 'task_type': detected_task_type
                    }, f)
            ...
            # Use train_and_evaluate_models with optimized parameters
            print(f"  🤖 Training models with optimized settings...")
            print(f"    ⚙️  n_estimators={n_estimators}, max_depth={max_depth if max_depth else 'adaptive'}, n_jobs={n_jobs}")
            
            # Call train_and_evaluate_models with optimization parameters
            model_results = train_and_evaluate_models(
                X_train, X_test, y_train, y_test, detected_task_type, target_col,
                n_estimators=n_estimators, max_depth=max_depth, early_stopping=early_stopping, n_jobs=n_jobs
            )
            ...
            for result in model_results:
                ...
                if hasattr(X_train, 'toarray'):
                    X_train_shap = X_train.toarray() if model_name == 'LogisticRegression' else X_train
                    X_test_shap = X_test.toarray() if model_name == 'LogisticRegression' else X_test
                else:
                ...
                shap_success, shap_importance, shap_df = perform_shap_analysis(
                    model, X_train_shap, X_test_shap, model_name,
            ...
            # Handle errors (if any models failed)
            failed_models = []
            for result in model_results:
                if 'error' in result:
            ...
            # Apply automatic fairness improvement if enabled
            if auto_improve_fairness and FAIRNESS_IMPROVEMENT_AVAILABLE:
                # Convert demographic groups to train/test format
                if demographic_column and demographic_column in df.columns:
                    demographic_train_values = df.loc[X_train.index if hasattr(X_train, 'index') else 
                                                     df.index[:len(X_train)], demographic_column].values
                    demographic_test_values = df.loc[X_test.index if hasattr(X_test, 'index') else 
                                                    df.index[len(X_train):len(X_train)+len(X_test)], demographic_column].values
                else:
                    # Synthetic groups
                    np.random.seed(42)
                    demographic_train_values = np.random.choice(['GroupA', 'GroupB'], size=len(X_train))
                    demographic_test_values = np.random.choice(['GroupA', 'GroupB'], size=len(X_test))
                
                # Convert to dict format
                demo_train_dict = {
                    'GroupA': demographic_train_values == 'GroupA',
                    'GroupB': demographic_train_values == 'GroupB'
                }
                demo_test_dict = {
                    'GroupA': demographic_test_values == 'GroupA',
                    'GroupB': demographic_test_values == 'GroupB'
                }
                
                # Apply fairness improvement
                improvement_results = integrate_fairness_improvement_into_pipeline(
                    fairness_df, models_dict, predictions_dict,
                    X_train, X_test, y_train, y_test,
                    demo_train_dict, demo_test_dict,
                    shap_results_dict, feature_names, detected_task_type,
                    auto_improve=True, disparity_threshold=disparity_threshold
                )
                
                # Update predictions with improved versions
                if improvement_results.get('improvement_applied', False):
                    predictions_dict = improvement_results['improved_predictions']
                    # Recalculate fairness with improved predictions
                    fairness_df_improved = perform_fairness_analysis(
                        models_dict, X_test, y_test, predictions_dict,
                        demographic_groups, detected_task_type
                    )
                    fairness_df_improved['Target'] = target_col
                    fairness_df_improved['Improvement_Applied'] = True
                    fairness_df_improved['Disparity_Before'] = improvement_results.get('disparity_before', np.nan)
                    fairness_df_improved['Disparity_After'] = improvement_results.get('disparity_after', np.nan)
                    fairness_df_improved['Improvement_Percent'] = improvement_results.get('improvement_percent', 0.0)
                    fairness_df = fairness_df_improved
                    
                    # Update metrics with improved predictions
                    for model_name in models_dict.keys():
                        if model_name in predictions_dict:
                            y_pred_improved = predictions_dict[model_name]
                            improved_metrics = calculate_all_metrics(y_test, y_pred_improved, detected_task_type)
                            metrics_dict[model_name] = improved_metrics
        ...
        # Call train_and_evaluate_models with optimization parameters
        model_results = train_and_evaluate_models(
            X_train, X_test, y_train, y_test, detected_task_type, target_columns[0],
            n_estimators=n_estimators, max_depth=max_depth, early_stopping=early_stopping, n_jobs=n_jobs
        )
        ...
        for result in model_results:
            ...
            # SHAP analysis
            shap_success, shap_importance, shap_df = perform_shap_analysis(
                result['model'], X_train_shap, X_test_shap, model_name,
                detected_task_type, feature_names
            )
            shap_results_dict[model_name] = {
                'success': shap_success,
        ...
        if auto_improve_fairness and FAIRNESS_IMPROVEMENT_AVAILABLE:
            ...
            if improvement_results.get('improvement_applied', False):
                ...
                fairness_df_improved['Target'] = target_columns[0]
                fairness_df_improved['Improvement_Applied'] = True
                fairness_df_improved['Disparity_Before'] = improvement_results.get('disparity_before', np.nan)
                fairness_df_improved['Disparity_After'] = improvement_results.get('disparity_after', np.nan)
                fairness_df_improved['Improvement_Percent'] = improvement_results.get('improvement_percent', 0.0)
